...
cast send --gas-limit 8000000 --rpc-url $rpc_url --private-key $USER_2_PK $MOCK_WETH_ADDRESS "approve(address,uint256)" $BALANCE_MANAGER_ADDRESS 10000000000000000000 > /dev/null 2>&1

# Deposit all WETH for USER to the BalanceManager contract
echo "Deposit WETH..."
cast send --gas-limit 8000000 --rpc-url $rpc_url --private-key $USER_2_PK $BALANCE_MANAGER_ADDRESS "deposit(address,uint256)" $MOCK_WETH_ADDRESS 10000000000000000000 > /dev/null 2>&1
...
# Approve allowance for BalanceManager contract for USER
echo "Approving allowance..."
cast send --gas-limit 8000000 --rpc-url $rpc_url --private-key $USER_3_PK $MOCK_USDC_ADDRESS "approve(address,uint256)" $BALANCE_MANAGER_ADDRESS 1000000000000 > /dev/null 2>&1

# Deposit all USDC for USER to the BalanceManager contract
echo "Deposit USDC..."
cast send --gas-limit 8000000 --rpc-url $rpc_url --private-key $USER_3_PK $BALANCE_MANAGER_ADDRESS "deposit(address,uint256)" $MOCK_USDC_ADDRESS 1000000000000 > /dev/null 2>&1
